...
export const InfiniteGridBackground = ({
  ...
  // On mobile — just render children, no grid/animations
  if (isMobile) {
    return <div className={cn("relative w-full", className)}>{children}</div>
  }

  return <DesktopGrid className={className}>{children}</DesktopGrid>
};

function DesktopGrid({ children, className }: { children?: React.ReactNode; className?: string }) {
  const containerRef = useRef<HTMLDivElement>(null);
...
const GridPattern = ({ offsetX, offsetY }: { offsetX: ReturnType<typeof useMotionValue<number>>; offsetY: ReturnType<typeof useMotionValue<number>> }) => (
  <svg className="w-full h-full">
    <defs>
      <motion.pattern id="quralhub-grid" width="40" height="40" patternUnits="userSpaceOnUse" x={offsetX} y={offsetY}>
